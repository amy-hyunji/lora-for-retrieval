...
from transformers import AutoModel, AutoTokenizer, Adafactor, AdamW
...
class FineTune_Contriever(pl.LightningModule):
    def __init__(self, hparams):
        ...
        if "roberta" in hparams.model_name_or_path:
            model = roberta_contriever.Contriever.from_pretrained(hparams.model_name_or_path)
        else:
            assert "bert" in hparams.model_name_or_path or "contriever" in hparams.model_name_or_path
            model = contriever.Contriever.from_pretrained(hparams.model_name_or_path)
        #model = AutoModel.from_pretrained(hparams.model_name_or_path)
        self.model = get_peft_model(model, peft_config)
        if hparams.lora_ckpt is not None:
            print(f"Loading lora ckpt from .. {hparams.lora_ckpt}")
            self.model = PeftModel.from_pretrained(self.model, hparams.lora_ckpt)
