...
class BiEncoderTrainer(object):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        saved_state = None
        if model_file:
            logger.info(f"[1] ***** Initializing model from {model_file} *****")
            saved_state = load_states_from_checkpoint(model_file)
            set_cfg_params_from_state(saved_state.encoder_params, cfg)
            saved_state_load_all = True
    
        """
        if cfg.encoder.pretrained_file != cfg.encoder.pretrained_model_cfg:
            logger.info(f"[2] ***** Initializing model from {cfg.encoder.pretrained_file} *****")
            saved_state = load_states_from_checkpoint(cfg.encoder.pretrained_file)
            set_cfg_params_from_state(saved_state.encoder_params, cfg)
            saved_state_load_all = False 
        """
        ...
        if saved_state:
            self._load_saved_state(saved_state, saved_state_load_all)
    ...
    def get_data_iterator(
        ...
        if type(hydra_datasets) != list:
            hydra_datasets = [hydra_datasets]
    ...
    def _load_saved_state(self, saved_state: CheckpointState, saved_state_load_all: bool):
        ...
        if saved_state_load_all:
            self.cfg.ignore_checkpoint_offset = False 
            self.cfg.ignore_checkpoint_optimizer = False 
            self.cfg.ignore_checkpoint_lr = False 
        else:
            self.cfg.ignore_checkpoint_offset = True 
            self.cfg.ignore_checkpoint_optimizer = True 
            self.cfg.ignore_checkpoint_lr = True 
        ...
        logger.info("Loading checkpoint @ batch=%s and epoch=%s", self.start_batch, self.start_epoch)
