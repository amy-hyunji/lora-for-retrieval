...
if transformers.__version__.startswith("4"):
    from transformers import BertConfig, BertModel, BertForSequenceClassification, RobertaModel, RobertaConfig
    from transformers import AdamW
    from transformers import BertTokenizer
    from transformers import RobertaTokenizer
else:
    from transformers.modeling_bert import BertConfig, BertModel, RobertaConfig, RobertaModel
...
from peft import get_peft_config, get_peft_model, LoraConfig, TaskType, get_peft_model_state_dict, set_peft_model_state_dict, PeftType, PrefixTuningConfig, PromptEncoderConfig, PromptTuningInit, PromptTuningConfig
...
def get_bert_biencoder_components(cfg, inference_only: bool = False, **kwargs):
    dropout = cfg.encoder.dropout if hasattr(cfg.encoder, "dropout") else 0.0
    peft_method=cfg.encoder.peft_method if hasattr(cfg.encoder, "peft_method") else "lora"
    # peft_method = "lora"
    # if peft_method in ["prompt_tuning", "lora"]: 
    #     question_encoder = HF_PEFT_BertEncoder.init_encoder(
    #         cfg.encoder.pretrained_model_cfg,
    #         use_peft=cfg.encoder.use_peft,
    #         peft_method=peft_method,
    #         projection_dim=cfg.encoder.projection_dim,
    ...
    #     )
    # else:
    logger.info(f"Initialize Q and CTX encoder from {cfg.encoder.pretrained_model_cfg}")
    ...
    # if peft_method in ["prompt_tuning", "lora"]:
    #     ctx_encoder = HF_PEFT_BertEncoder.init_encoder(
    #         cfg.encoder.pretrained_model_cfg,
    #         use_peft=cfg.encoder.use_peft,
    #         peft_method=peft_method,
    #         projection_dim=cfg.encoder.projection_dim,
...
def get_bert_reader_components(cfg, inference_only: bool = False, **kwargs):
    assert False, f"NOT using reader component for now"
    dropout = cfg.encoder.dropout if hasattr(cfg.encoder, "dropout") else 0.0
...
class HFRobertaEncoder(RobertaModel):
    ...
    @classmethod
    def init_encoder(
        cls, cfg_name: str, use_peft: bool = False, peft_method: str = "lora", projection_dim: int = 0, dropout: float = 0.1, pretrained: bool = True, **kwargs
    ) -> RobertaModel:
        ...
        if pretrained:
            if not use_peft:
                print(f"## Training FULL parameter!")
                return cls.from_pretrained(cfg_name, config=cfg, project_dim=projection_dim, **kwargs)
            else:
                print(f"## Training with peft method -> {peft_method}")
                # if peft_method in ["lora", "prompt_tuning", "prefix_tuning"] or peft_method is None or peft_method == "None":
                #     if peft_method == "lora":
                #         peft_config = LoraConfig(
                #             r=8, lora_alpha=32, lora_dropout=0.1#, task_type=TaskType.TOKEN_CLS
                #         )
                ...
                #         peft_config = PrefixTuningConfig(num_virtual_tokens=10)
                #     else:
                #         assert False
            ...
            assert False, "Use the pretrained ckpt"
...
class HF_PEFT_BertEncoder(BertModel):
    def __init__(self, config, project_dim: int = 0):
    ...
    def init_encoder(
        ...
        if pretrained:
            if not use_peft:
                print(f"## Training FULL parameter! - {cfg_name}")
                return cls.from_pretrained(cfg_name, config=cfg, project_dim=projection_dim, **kwargs)
            else:
                print(f"## Training with peft method -> {peft_method}")
                if peft_method == "lora":
                    ...
                    peft_config = PromptTuningConfig(
                        ...
                        tokenizer_name_or_path=cfg_name,
                    )
...
class HFBertEncoder(BertModel):
    ...
    @classmethod
    def init_encoder(
            cls, cfg_name: str, use_peft: bool = False, peft_method: str = "lora", projection_dim: int = 0, dropout: float = 0.1, pretrained: bool = True, pretrained_file: str = "", **kwargs
    ) -> BertModel:
        logger.info("Initializing HF BERT Encoder. cfg_name=%s", cfg_name)
        cfg = BertConfig.from_pretrained(cfg_name if cfg_name else "bert-base-uncased")
        if dropout != 0:
    ...
    def init_encoder(
        ...
        if pretrained:
            if not use_peft:
                ...
                print(f"## Training with peft method -> {peft_method}")
                if peft_method in ["lora", "prompt_tuning", "prefix_tuning"]:
                    if peft_method == "lora":
                        peft_config = LoraConfig(
                            r=8, lora_alpha=32, lora_dropout=0.1#, task_type=TaskType.TOKEN_CLS
                        )
