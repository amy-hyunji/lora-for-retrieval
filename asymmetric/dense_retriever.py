...
def beir_evaluate(qrels: Dict[str, Dict[str, int]], 
...
def save_results(
    ...
    qrel = {}; run = {}
    for qnum, elem in enumerate(merged_data):
        qid = f"q{qnum}"
        ...
        preds = elem["ctxs"]
        for pred in preds:
            pid = pred["id"].replace("abstract:", "")
            score = pred["score"]
            run[qid][str(pid)] = float(score)

    print(beir_evaluate(qrel, run, [1,3,5,10,100]))
    ...
    # ret = evaluator.evaluate(run)
    # total_metric = defaultdict(list)

    # for q, vdict in ret.items():
    #     for k, v in vdict.items():
    #         total_metric[k].append(v*100)

    # for k, v in total_metric.items():
    #     print(f"[{k}]: {round(np.mean(np.array(v)), 2)}")
    #     logger.info(f"[{k}]: {round(np.mean(np.array(v)), 2)}")
    

    logger.info("Save results * scores  to %s", out_file)
...
def main(cfg: DictConfig):
    ...
    if cfg.rpc_retriever_cfg_file and cfg.rpc_index_id:
        ...
        id_prefixes = []
        ctx_sources = []
        if type(cfg.ctx_datatsets) != list:
            cfg.ctx_datatsets = [cfg.ctx_datatsets]
        ...
        if type(ctx_files_patterns) != list:
            ctx_files_patterns = [ctx_files_patterns]
